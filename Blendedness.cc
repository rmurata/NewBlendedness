...
void computeMoments(
    ...
    for (SpanIter spanIter = region.begin(); spanIter != spanEnd; ++spanIter) {
        ...
        for (PointIter pointIter = span.begin(); pointIter != pointEnd; ++pointIter, ++pixelIter) {
            ...
            float mu = ( (std::sqrt(variance/(2.0f*boost::math::constants::pi<float>()))* //fixed by Murata. (2/pi) --> (2xpi)
			std::exp(-0.5f*(data*data)/variance)) +
			 0.5f*data*boost::math::erfc(-data/std::sqrt(2.0f*variance)) )/(1.0f - 0.5f*boost::math::erfc(data/std::sqrt(2.0f*variance)));
	    //fixed by Murata. add normalization.
	    ...
	    std::max( accumulatorAbs(d.getX(), d.getY(), weight, std::abs(data) - bias ) ); // fixed by Murata. the location of np.max.
        }
    }
}
